...
df_soortentabel <- df_soortenlijst %>% 
  ...
  summarise(n_input_seqs = sum(input_data$TAXID== taxid),
            n_amplicons = sum(ecopcr_combined$taxid == taxid),
            n_determined = sum(ecopcr_combined$obi_taxid == taxid))
...
df_multihits <- 
  ...
  do({
    ...
    data.frame(multihit_taxa = paste(sort(unique(taxids)), collapse = ','))
  })
...
df_soortenevaluatie <- df_soortentabel %>% 
  ...
  mutate(pref_taxid = ifelse(merged == FALSE, taxid, pref_taxid)) %>% 
