...
system2("powershell", args = pscommand_build_riaz)#Run de container van de imag
...
##copy to container
system2('docker',  c('cp', 
                     file.path("database", db_name_riaz, obi_script_riaz), 
                     paste0(docker_container_name, ':', docker_script_riaz)))
system2('docker',  c('cp', taxdump_name, docker_taxonomy))
system2('docker',  c('cp', file.path("database", db_name_riaz, fasta_name), docker_input_fasta)) 

##execute main script
system2('docker', c("exec", docker_container_name, "chmod", "777", docker_script_riaz)) #???
system2('docker', c('exec', docker_container_name, 'bash', '-c', docker_script_riaz))

##return results from container to windows
system2('docker', c("cp", paste0(docker_path, "logfile.txt"), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, obi_script_riaz), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, "kept_input.fasta"), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, "amplified.fasta"), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, "amplified_uniq.fasta"), refdb_location_riaz))
#system2('docker', c("cp", paste0(docker_path, "amplified_clean.fasta"), refdb_location_riaz))
#system2('docker', c("cp", paste0(docker_path, "amplified_clean_uniq.fasta"), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, "final_db_0.97.fasta"), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, "final_db_0.99.fasta"), refdb_location_riaz))
system2('docker', c("cp", paste0(docker_path, "refdb.obidms"), refdb_location_riaz))        
